...
Uses Bybit public API (no key required, no geo-restrictions).
Replaces Binance which returns 451 in the US.
"""
...
SERVICE_NAME = "crypto-orderbook MCP (Bybit)"

BYBIT_URL = "https://api.bybit.com"
TIMEOUT = 10.0

# Map common crypto tickers to Bybit trading pairs
...
def _to_pair(symbol: str) -> str:
    """Convert a ticker symbol to a Bybit trading pair."""
        ...
        for sym in symbols[:15]:
            ...
            try:
                resp = await client.get(
                    f"{BYBIT_URL}/v5/market/orderbook",
                    params={"category": "spot", "symbol": pair, "limit": 25},
                )
                resp.raise_for_status()
                data = resp.json()
                book = data.get("result", {})

                # Bybit v5 response: {"result": {"b": [[price, size], ...], "a": [[price, size], ...]}}
                bids = book.get("b", [])
                asks = book.get("a", [])

                # Sum bid and ask depth (price * quantity for top levels)
                ...
                # Midpoint price from best bid/ask
                ...
                logger.warning("Bybit orderbook error for %s: %s", sym, exc)
    ...
    async with httpx.AsyncClient(timeout=TIMEOUT) as client:
        for sym in symbols[:15]:
            ...
            try:
                resp = await client.get(
                    f"{BYBIT_URL}/v5/market/orderbook",
                    params={"category": "spot", "symbol": pair, "limit": 50},
                )
                resp.raise_for_status()
                data = resp.json()
                book = data.get("result", {})
                ...
                logger.warning("Bybit depth error for %s: %s", sym, exc)
