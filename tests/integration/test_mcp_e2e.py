...
Execute manually: ``python tests/integration/test_mcp_e2e.py``
Or inside compose: ``docker compose exec cuga python tests/integration/test_mcp_e2e.py``
"""
...
def run_mcp_pipeline_smoke() -> None:
    """Run full MCP → normalizer → Redis → merger smoke (requires live stack)."""
...
@pytest.mark.e2e
@pytest.mark.skip(reason="Requires live MCP stack; run via python tests/integration/test_mcp_e2e.py")
def test_mcp_pipeline_smoke() -> None:
    """Placeholder so CI can exclude e2e; run run_mcp_pipeline_smoke() manually."""
    run_mcp_pipeline_smoke()
