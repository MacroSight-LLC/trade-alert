...
for i in $(seq 1 30); do
    HTTP_CODE=$(wget --spider -qS http://127.0.0.1:8200/v1/sys/health 2>&1 | grep "HTTP/" | awk '{print $2}' | tail -1 || echo "000")
...
if [ -f "$INIT_FILE" ]; then
    # Check if sealed using wget + grep (no python dependency)
    HEALTH=$(wget -qO- http://127.0.0.1:8200/v1/sys/health 2>/dev/null || true)
    IS_SEALED=$(echo "$HEALTH" | grep -o '"sealed":true' || true)
    if [ -n "$IS_SEALED" ]; then
        # Extract unseal key using sed (no python dependency)
        UNSEAL_KEY=$(cat "$INIT_FILE" | sed -n 's/.*"unseal_keys_b64":\["\([^"]*\)".*/\1/p')
